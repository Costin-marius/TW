...
//implementati export (se vor extrage parintii cu copiii lor de forma GET / care va intoarce in body un array de university unde university contine 
// un array de students si courses plus enrollements ca perechi de id student si course
...
application.get("/", async (request, response, next) => {
  try {
    ...
    const result = data.map(u => ({
      ...
      enrollements: u.students.flatMap(s =>
        s.courses.map(c => ({
          studentId: s.id,
          courseId: c.id
        }))
